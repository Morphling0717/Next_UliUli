...
本分支固定安装 `vendor/windchime-embed-0.6.1.tgz`，与 Mia 使用同一压缩包。共享库源码在 WindChime 仓库；本网站提供登录、页面和数据库连接，不维护另一套播出审核规则。代码保留在 `codex/windchime-desktop-v0.6.0` 分支，未合并或修改 `main`。
...
网站必须安装并实际部署 0.6.1 才能生成和验证密钥；只更新桌面应用或仓库依赖不会升级当前运行的旧站点。2026-09-13 经用户授权，UliUli 已部署 `89a3468`，完成数据库迁移及公网验证，见 [生产部署记录](WINDCHIME-PRODUCTION-20260913.md)。本轮未在生产生成测试密钥或合成信件，Mia 未在此次服务器部署范围内。
...
回退代码时先确认旧镜像与迁移后的数据库兼容；本次加法迁移已在副本上验证兼容，可保留当前数据库，避免覆盖升级后的真实写入。只有需要恢复数据时，才在停止全部写入后核对并恢复一致的数据库/图片备份，不手工删列或覆盖运行中的数据库。具体回退步骤见生产部署记录。
