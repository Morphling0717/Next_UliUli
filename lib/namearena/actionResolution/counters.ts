...
export function handleWaitCounter(
  ...
  runtime.log('info', `🚫 ${user.name} 的当前一击被打断了！`);
  ...
  if (actualCounterDmg > 0) {
    ...
    runtime.log('crit', `💥 强力反击成功命中 ${user.name}；但【黄昏余命】期间未再损失生命！`);
  } else {
...
export function handleCounterStatus(
  ...
  if (counterType === 'CTR_CHARM') {
    ...
    runtime.log('info', `😍 【魅惑反击】${target.name} 反制 ${user.name}，${user.name} 被魅惑 2 回合，当前一击中断！`);
    return true;
  }
  if (counterType === 'CTR_STUN') {
    ...
    runtime.log('info', `💫 【震慑反击】${target.name} 震慑 ${user.name}，${user.name} 眩晕 2 回合，当前一击中断！`);
    return true;
  }
  if (counterType === 'CTR_DRAIN') {
    ...
    runtime.flushDeferredDamageEvents(user, 'mitigation');
    const healing = resolveHealing(target, drain, { kind: 'lifesteal', sourceId: '汲取反击', healer: target }, runtime.log);
    if (connected && drain <= 0) {
      runtime.log('info', `🧛 【汲取反击】${target.name} 成功命中 ${user.name}；但【黄昏余命】期间没有可汲取的生命，因此未获得治疗！`);
    } else if (drain <= 0) {
      runtime.log('info', `🧛 【汲取反击】${target.name} 试图吸取 ${user.name} 的生命，但没有吸到有效生命！`);
    } else if (healing.actual > 0) {
      runtime.log('heal', `🧛 【汲取反击】${target.name} 吸取 ${user.name}，${user.name} 实际损失 ${drain} 点生命，${target.name} 恢复了 ${healing.actual} 点生命！`);
    } else if (healing.outcome === 'blocked') {
      runtime.log('info', `🧛 【汲取反击】${target.name} 吸取 ${user.name}，${user.name} 实际损失 ${drain} 点生命，但 ${target.name} 的治疗被完全阻止！`);
    } else {
  ...
  if (counterType === 'CTR_FREEZE') {
    ...
    runtime.log('poison', `🧊 【极寒反击】${target.name} 冰封 ${user.name}，${user.name} 冻结 2 回合，当前一击中断！`);
    return true;
  }
  if (counterType === 'CTR_VOID') {
    ...
    if (actualDmg > 0) {
      ...
      runtime.log('crit', `🌌 【虚空反击】虚空陷阱成功吞噬 ${user.name}；但【黄昏余命】期间未再损失生命！`);
    } else {
  ...
  if (counterType === 'CTR_WEAK') {
    if (runtime.applyStatus(user, { identityId: 'WEAK', remainingTurns: 2 })) {
  ...
  if (counterType === 'CTR_CONFUSE') {
    ...
    runtime.log('info', `🌀 【混乱反击】${target.name} 扭曲 ${user.name} 的判断，${user.name} 混乱 3 回合，当前一击中断！`);
    return true;
  }
  if (counterType === 'CTR_EXECUTE') {
    if (user.hpPct < 0.4) {
      ...
      if (!defeated && user.currentHp > 0 && !user.isDead && !user.isDeadAnnounced) {
        runtime.log('info', `☠️ 【断头反击】${target.name} 的断头台已经落下，但 ${user.name} 的保命机制强行改写了处决结果，当前一击中断！`);
      }
      return true;
    }
