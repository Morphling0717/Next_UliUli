...
// 主站首页客户端入口（原 app/page.tsx）。
// 现在 app/page.tsx 是 server component，会从 DB 读初始 siteConfig/songs 注入到这里，
// 让首屏 HTML 在不跑 JS 的情况下也能带完整文案 + 关键词，国内爬虫（百度/360/搜狗）
// 也能拿到结构化的 hero/credits/歌单等内容。
//
// 这里仍然保留 /api/config 轮询：管理员后台改完内容后 ~15s 内自动注水覆盖 SSR
// 的初始值，等同原来体验。
...
import { ToastContainer, GoldenLuckModal } from './UI';
...
export interface HomeClientProps {
  ...
  initialHiddenSongs: SongSystemSongItem[];
}
...
export default function HomeClient({
  ...
  const [hiddenSongs, setHiddenSongs] = useState<SongSystemSongItem[]>(initialHiddenSongs);
  ...
  useEffect(() => {
    ...
    // 启动 15 秒轮询；首次不立刻 fetch（SSR 已经塞了初始值），
    // 等第一个 interval 到期再覆盖。
